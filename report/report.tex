...
\usepackage{geometry}
\usepackage{graphicx} % Essential for the logo
...
\begin{titlepage}
    ...
    % --- HEADER ---
    \vspace*{1cm} % Visual top margin
    
    {\scshape\LARGE University of Trento \par}
    
    \vspace{1.5cm} % Fixed spacing between institution and logo
    
    % Logo: inserted directly without a figure environment
    % width=0.35\textwidth is a good visual ratio for vertical/square logos
    ...
    % --- TITLE BLOCK ---
    % Horizontal rules add structure.
    % Added a bit of vertical breathing room between the rules.
    \hrule height 0.5pt
    \vspace{0.6cm}
    ...
    \vfill % Flexible space 3 (pushes the date to the bottom)
    
    % --- DATE (Month and Year only) ---
    % This generates the month name in English plus year
    {\large 
    ...
    \vspace*{2cm} % Visual bottom margin
\begin{center}
...
\begin{itemize}
    \item \textbf{BDI Architecture Implementation:} Development of a full BDI loop that manages belief revision (handling old data for parcels and agents) and generates options based on a dynamic utility function that considers reward decay and contention penalty.
...
\begin{itemize}
    ...
    \item \textbf{Belief Model (`src/bdi/belief.js`):} Act as the agent's memory, storing the current state of the environment (parcels, agents, map configuration) and managing the expiration of old data.
...
\noindent The \texttt{launcher.js} file serves as the entry point, initializing these components and wiring the server events (sensing) to the belief revision functions.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Claims:} A registry of intentions broadcasted by the partner agent. These have a Time-To-Live (TTL) of 3 seconds to avoid old locks on parcels.
...
\begin{itemize}
    ...
    \item $Cost_{move} = D_{total} \times loss \times (carried + 1)$, where $D_{total}$ is the sum of the Manhattan distance to the parcel and the distance from the parcel to the nearest delivery zone.
...
\begin{enumerate}
    ...
    \item \textbf{PROTOCOL\_RELEASE:} If no pocket is available but the agent is carrying parcels needed by the other side, it drops the parcels and retreats, signaling the partner to take over.
    \item \textbf{PROTOCOL\_ACQUIRE:} The partner waits for the path to clear, moves to the dropped parcels, and collects them.
    \item \textbf{PROTOCOL\_STUCK}: Triggered when yield fails (no free adjacent cell). 
          The partner attempts to yield instead; if both fail, the protocol terminates 
          with a randomized backoff (1-3s) to prevent repeated deadlock attempts.
    \item \textbf{PROTOCOL\_END}: Signals protocol completion and resets coordination state.
\end{enumerate}
...
\noindent Data collection was automated using the custom \texttt{RunLogger} module, which recorded score progression, solver duration, and event processing times for every run.
...
\noindent Two distinct agent configurations were tested:
\begin{itemize}
...
\noindent For completeness, the online PDDL solver was also tested. While functionally correct, it introduced a noticeably higher planning latency compared to the local solver, so it was not used in the main benchmarks.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l|rr|r|rr|r}
        ...
        Hallway & 0 / 169 & 169 & 0/100 & 0 / 233 & \textbf{233} & 0/100 \\
        \bottomrule
    \end{tabular}
    \endgroup
\end{table}
...
\noindent Crucially, Figure \ref{fig:lat_single} shows that this cognitive overhead does not propagate to the overall loop latency. The average cycle time remains comparable between BFS and PDDL configurations (mostly in the \textbf{50--120 ms} range).
...
\noindent The online PDDL solver was also validated during development; although it produced valid plans, its network latency was significantly higher than the local solver, which would degrade real-time responsiveness.
...
\noindent In conclusion, \textit{ParcelPredator} successfully demonstrates that a BDI agent, when supported by optimized engineering solutions like local solving and caching, can exhibit complex, intelligent behavior in real-time environments, effectively balancing individual optimality with team cooperation.
